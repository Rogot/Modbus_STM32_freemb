...
t_modbus_him_pack rxData = {0};
t_modbus_him_pack txData = {0};
...
xMBPortSerialInit( UCHAR ucPORT, ULONG ulBaudRate, UCHAR ucDataBits, eMBParity eParity )
{
	  ...
	  rxData.start_address = 0x0;
		rxData.data_size = 0x0;
	
		for (uint8_t i = 0; i < MB_HMI_PACKET_DATA_SIZE; i++) {
			rxData.data[i] = 0x0;
		}			
		rxData.crc = 0x00;
...
xMBPortSerialGetByte( CHAR * pucByte )
{
     ...
     */
		...
		//*pucByte = modbusUart->Instance->DR;
